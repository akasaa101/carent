  ...
  include 'header.php';
  if (empty($_GET['sef'])) {
    header('locaiton:index');
  } elseif (empty($_GET['seff'])) {
...
<div class="select-cars">
  <div class="container">
    ...
    $aracsor=$db->prepare("SELECT * FROM arac where ofis_alisid=:ofis_alisid and ofis_teslimid=:ofis_teslimid order by arac_sira ASC");
    $aracsor->execute(array(
      'ofis_alisid' => $_GET['sef'],
      'ofis_teslimid' => $_GET['seff']
    ));

...
$vgsor=$db->prepare("SELECT * FROM vg where vg_durum=:vg_durum");
$vgsor->execute(array(
  'vg_durum' => 1
));
$say=$vgsor->rowCount();
...
<head>
  <title>Video Galeri - <?php echo $ayarcek['ayar_title'] ?></title>
  ...
  <meta name="keywords" content="video galeri,video,galeri" />
  ...
  <div class="header header-style-2">
    <div class="container">
      ...
      <div class="col-md-12">
        <div class="iletisim2">
          <center>
            <h1 style="color: white;">Video Galeri</h1>
          </center>
        </div>
      </div>
    </div>
...
<section class="blog-posts">
  <div class="container">
    <div class="row">
      <?php
      $vgsor=$db->prepare("SELECT * FROM vg where vg_durum=1 order by vg_sira ASC");
      $vgsor->execute();
      $say=$vgsor->rowCount();
      while ($vgcek=$vgsor->fetch(PDO::FETCH_ASSOC)) {
        ...
        <div class="col-md-4">
          <div class="blog">
            <video controls name="media" width="300px" height="300px">
    ...
    <?php 
    if ($say==0) { ?>
      <h3><center><p>Henüz bir Videomuz bulunmamaktadır.</p></center></h3>
    <?php } ?>
